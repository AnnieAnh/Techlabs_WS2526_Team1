...
# Run without LLM — LLM columns will be None/empty, regex fields only
poetry run python orchestrate.py --no-llm
```

> **`--no-llm` mode:** When `state.no_llm` is set, `clean_enrich` forces `results = []` regardless of whether `extraction_results.json` exists on disk. This prevents stale LLM data from a previous run leaking into a no-LLM pipeline. List columns (`technical_skills`, etc.) become `"[]"` via `standardize_missing_values`; string LLM columns become `None`.
...
See [Analysis Documentation](../analysis/ANALYSIS.md) for the next stage.
